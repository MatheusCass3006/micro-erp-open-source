...
export class NotaService {
  ...
  private get notaRepo() { return AppDataSource.getRepository(Nota); }
  private get itemRepo() { return AppDataSource.getRepository(ItemNota); }
  ...
  async resumoMensal(empresaId: number, mes?: number, ano?: number) {
    ...
    // SQLite usa strftime — MONTH()/YEAR() são funções MySQL/PostgreSQL
    const resultado = await this.notaRepo
      .createQueryBuilder("nota")
      .select("SUM(nota.valorTotal)", "total")
      .addSelect("COUNT(nota.id)", "quantidade")
      .where("nota.empresaId = :empresaId", { empresaId })
      .andWhere("strftime('%m', nota.dataEntrada) = :mes", { mes: String(mes).padStart(2, "0") })
      .andWhere("strftime('%Y', nota.dataEntrada) = :ano", { ano: String(ano) })
      .getRawOne();
