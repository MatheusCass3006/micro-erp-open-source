...
export class SaidaService {
  private get saidaRepo() {
    return AppDataSource.getRepository(Saida);
  }
  private get categoriaRepo() {
    return AppDataSource.getRepository(CategoriaSaida);
  }
  ...
  async listar(
    ...
    if (mes) {
      qb.andWhere("strftime('%m', saida.data) = :mes", { mes: String(mes).padStart(2, '0') });
    }
    if (ano) {
      qb.andWhere("strftime('%Y', saida.data) = :ano", { ano: String(ano) });
    }
    if (secao) {
  ...
  async atualizar(saidaId: number, empresaId: number, dados: any) {
    const saida = await this.saidaRepo.findOne({
      where: { id: saidaId, empresaId, deletadoEm: IsNull() },
    });
  ...
  async resumoMensal(empresaId: number, mes?: number, ano?: number) {
    ...
    const resultado = await this.saidaRepo
      ...
      .where("saida.empresaId = :empresaId", { empresaId })
      .andWhere("saida.deletadoEm IS NULL")
      .andWhere("strftime('%m', saida.data) = :mes", { mes: String(mes).padStart(2, '0') })
      .andWhere("strftime('%Y', saida.data) = :ano", { ano: String(ano) })
